   ...
   * @param pair the input {@code SubscriptPair} object.
	...
	public BanerjeeTest (SubscriptPair pair)
	{
		...
		for (Loop loop: loopnest)
		{
			...
			/* Check if coefficients of index variables are constant integer values */
			...
			else
			{
				...
				N = ((-1)*N);
                                ...
                                bounds.add(BanerjeeTest.LB_any, new Long((getNegativePart(A)-getPositivePart(B))*(U-L)+(A-B)*L));
                                bounds.add(BanerjeeTest.LB_less, new Long(getNegativePart (A-getPositivePart(B))*(U-L-N)+(A-B)*L+A*N));
                                bounds.add(BanerjeeTest.LB_equal, new Long(getNegativePart(A-B)*(U-L)+(A-B)*L));
                                bounds.add(BanerjeeTest.LB_greater, new Long(getNegativePart(getNegativePart(A)-B)*(U-L-N)+(A-B)*L - B*N));

                                bounds.add(BanerjeeTest.UB_any, new Long((getPositivePart(A)-getNegativePart(B))*(U-L)+(A-B)*L));
                                bounds.add(BanerjeeTest.UB_less, new Long(getPositivePart (A-getNegativePart(B))*(U-L-N)+(A-B)*L+A*N));
                                bounds.add(BanerjeeTest.UB_equal, new Long(getPositivePart(A-B)*(U-L)+(A-B)*L));
                                bounds.add(BanerjeeTest.UB_greater, new Long(getPositivePart(getPositivePart(A)-B)*(U-L-N)+(A-B)*L - B*N));
			}
	...
	public boolean testDependence(DependenceVector dependence_vector)
	{
		...
		for (Loop loop : nest)
		{
			...
			banerjeeLB += banerjee_bounds.get(loop).get(loop_dependence_direction+BanerjeeTest.LB);
			banerjeeUB += banerjee_bounds.get(loop).get(loop_dependence_direction+BanerjeeTest.UB);
		}
	...
	private void printDirectionVector(HashMap<Loop,Integer> dv, LinkedList<Loop> nest)
	{
		PrintTools.print("(", 2);
