 ...
 * 
 */
public abstract class AbstractNonKernelFunctionsImpl implements
		INonKernelFunctions {
	// /////////////////////////////////////////////////////////////////
	// Constants

	private final static Logger LOGGER = Logger
			.getLogger(AbstractNonKernelFunctionsImpl.class);
	 ...
	 * and
	 * {@link AbstractNonKernelFunctionsImpl#getExpressionsForVariables(List, EOutputGridType)}
	 * .
	 */
	...
	private void initialize() {
		...
		int nIdx = 0;
		for (GlobalGeneratedIdentifiers.Variable var : m_data.getData()
				.getGlobalGeneratedIdentifiers().getVariables()) {
			switch (var.getType()) {
			case INPUT_GRID:
			case OUTPUT_GRID:
	...
	/**
	 * Returns the expression that substitutes a {@link Variable} in the driver
	 * code.
	 * 
	 * @param variable
	 *            The variable for which to retrieve the Cetus HIR expression.
	 * @param typeOutputGrid
	 *            The form in which the expressions corresponding to
	 *            {@link EVariableType#OUTPUT_GRID} type variables are desired:
	 *            as regular identifiers or as pointers
	 * @return The HIR expression corresponding to <code>variable</code>
	 */
	public Expression getExpressionForVariable(
			GlobalGeneratedIdentifiers.Variable variable,
			EOutputGridType typeOutputGrid) {
		initialize();
	 ...
	 *            The variable for which to retrieve the Cetus HIR expression.
	 * @return The HIR expression corresponding to <code>variable</code>.
	 *         {@link EVariableType#OUTPUT_GRID} type variables are returned as
	 *         regular identifiers.
	 */
	public Expression getExpressionForVariable(
			GlobalGeneratedIdentifiers.Variable variable) {
		return getExpressionForVariable(variable,
	 ...
	 * @return
	 */
	public List<Expression> getExpressionsForVariables(
			List<Variable> listVariables, EOutputGridType typeOutputGrid) {
	...
	public List<Expression> getExpressionsForVariables(
		...
		return getExpressionsForVariables(listVariables,
				EOutputGridType.OUTPUTGRID_DEFAULT);
	}
	...
	public StatementList forwardDecls() {
		...
		GlobalGeneratedIdentifiers ids = m_data.getData()
				.getGlobalGeneratedIdentifiers();
		VariableDeclaration declInit = ids
				.getInitializeFunctionDeclaration(m_kernelSourceFile
						.getCompatibility());
		VariableDeclaration declStencil = ids
				.getStencilFunctionDeclaration(m_kernelSourceFile
						.getCompatibility());
			...
			if (var.isGrid()) {
				...
				if (exprId instanceof Identifier) {
					...
					if (var.getType() == GlobalGeneratedIdentifiers.EVariableType.INPUT_GRID)
						decl = new VariableDeclaration(var.getSpecifiers(),
								(VariableDeclarator) ((Identifier) exprId)
										.getSymbol());
					else if (var.getType() == GlobalGeneratedIdentifiers.EVariableType.OUTPUT_GRID) {
						decl = new VariableDeclaration(ASTUtil.dereference(var
								.getSpecifiers()),
								(VariableDeclarator) ((Identifier) exprId)
										.getSymbol());
					}
						...
						if (m_data.getOptions().getCreateValidationCode()) {
							sl.addDeclaration(new VariableDeclaration(
									...
									new VariableDeclarator(
											new NameID(
													StringUtil
															.concat(((Identifier) exprId)
																	.getName(),
																	ValidationCodeGenerator.SUFFIX_REFERENCE)))));
						}
					}
				}
			}
...
/** 
 * This methode generates the CommandlineParse code in the driver.c file
 * @return
 */
	public StatementList parseCommandLine() {
		...
		// int nParamArgsCount = 0;
		VariableDeclarator declParamArgsCount = new VariableDeclarator(
				new NameID("nParamArgsCount"));
		declParamArgsCount.setInitializer(new ValueInitializer(Globals.ZERO
				.clone()));
		sl.addStatement(new DeclarationStatement(new VariableDeclaration(
				Specifier.INT, declParamArgsCount)));

		// char** rgArgs = (char**) malloc (argc * sizeof (char*));
		VariableDeclarator declArgs = new VariableDeclarator(new NameID(
				"rgArgs"));
		declArgs.setInitializer(new ValueInitializer(
				new Typecast(
						CodeGeneratorUtil.specifiers(Specifier.CHAR,
		...
		sl.addStatement(new DeclarationStatement(new VariableDeclaration(
				CodeGeneratorUtil.specifiers(Specifier.CHAR,
						PointerSpecifier.UNQUALIFIED,
						PointerSpecifier.UNQUALIFIED), declArgs)));
		...
		cmpstmtIfBody1.addStatement(new ExpressionStatement(
				new AssignmentExpression(new ArrayAccess(new Identifier(
						declArgs), new Identifier(declParamArgsCount)),
						AssignmentOperator.NORMAL, new ArrayAccess(new NameID(
								"argv"), new Identifier(declI)))));
		// for (int i = 1; i < argc; i++)
		// if (*(argv[i]) != '-') ...
		cmpstmtIfBody1.addStatement(new ExpressionStatement(
				new UnaryExpression(UnaryOperator.PRE_INCREMENT,
						new Identifier(declParamArgsCount))));
		sl.addStatement(new ForLoop(new ExpressionStatement(
				new AssignmentExpression(new Identifier(declI),
						AssignmentOperator.NORMAL, Globals.ONE.clone())),
				new BinaryExpression(new Identifier(declI),
						BinaryOperator.COMPARE_LT, new NameID("argc")),
				new UnaryExpression(UnaryOperator.PRE_INCREMENT,
						new Identifier(declI)),
				new IfStatement(new BinaryExpression(new UnaryExpression(
						UnaryOperator.DEREFERENCE, new ArrayAccess(new NameID(
								"argv"), new Identifier(declI))),
						BinaryOperator.COMPARE_NE, new CharLiteral('-')),
						cmpstmtIfBody1)));
		...
		cmpstmtIfBody2.addStatement(new ExpressionStatement(new FunctionCall(
				new NameID("printf"), CodeGeneratorUtil.expressions(
						new StringLiteral(sbMsg.toString()), new ArrayAccess(
								new NameID("argv"), new IntegerLiteral(0))))));
		cmpstmtIfBody2.addStatement(new ExpressionStatement(new FunctionCall(
				new NameID("exit"), CodeGeneratorUtil
						.expressions(new IntegerLiteral(-1)))));

		sl.addStatement(new IfStatement(new BinaryExpression(new Identifier(
				declParamArgsCount), BinaryOperator.COMPARE_NE,
				new IntegerLiteral(m_mapCommandLineParamIndices.size())),
				cmpstmtIfBody2));

		// <paramname> = atoi (rgArgs[i]); : i=1..#args
		for (Variable var : m_data
				.getData()
				.getGlobalGeneratedIdentifiers()
				.getVariables(
						GlobalGeneratedIdentifiers.EVariableType.SIZE_PARAMETER
								...
								| GlobalGeneratedIdentifiers.EVariableType.INTERNAL_AUTOTUNE_PARAMETER
										.mask()
								| GlobalGeneratedIdentifiers.EVariableType.INTERNAL_NONKERNEL_AUTOTUNE_PARAMETER
										.mask())) {
			VariableDeclarator decl = new VariableDeclarator(new NameID(
					var.getName()));
			decl.setInitializer(new ValueInitializer(new FunctionCall(
					new NameID("atoi"), CodeGeneratorUtil
							.expressions(new ArrayAccess(new Identifier(
									declArgs), new IntegerLiteral(
									m_mapCommandLineParamIndices.get(var)))))));
			sl.addStatement(new DeclarationStatement(new VariableDeclaration(
					Globals.SPECIFIER_SIZE, decl)));
		}

		// free (rgArgs);
		sl.addStatement(new ExpressionStatement(new FunctionCall(new NameID(
				"free"), CodeGeneratorUtil
				.expressions(new Identifier(declArgs)))));
	 ...
	 * Subclasses can invoke/override if necessary. By default, this method is
	 * called in {@link AbstractNonKernelFunctionsImpl#declareGrids()}.
	 * 
	 * @return A statement list that provides the code for parsing the command
	 *         line or <code>null</code> if no parsing is required
	 */
	@Deprecated
	public StatementList parseCommandLine_Old() {
		...
		sl.addStatement(new IfStatement(new BinaryExpression(
				new NameID("argc"), BinaryOperator.COMPARE_NE,
				new IntegerLiteral(m_mapCommandLineParamIndices.size() + 1)),
				cmpstmtIfBody));

		/*
		 * VariableDeclarator declParams = new VariableDeclarator (new NameID
		 * ("rgParams"), ArraySpecifier.UNBOUNDED); m_idParams = new Identifier
		 * (declParams); List<Expression> listExprs = new ArrayList<Expression>
		 * (m_mapCommandLineParamIndices.size ()); for (int i = 0; i <
		 * m_mapCommandLineParamIndices.size (); i++) listExprs.add (new
		 * FunctionCall (new NameID ("atoi"), CodeGeneratorUtil.expressions (new
		 * ArrayAccess (new NameID ("argv"), new IntegerLiteral (i + 1)))));
		 * declParams.setInitializer (new Initializer (listExprs));
		 * sl.addStatement (new DeclarationStatement (new VariableDeclaration
		 * (Specifier.INT, declParams)));
		 * 
		 * // declare and assign size variables for (Variable varSize :
		 * m_data.getData ().getGlobalGeneratedIdentifiers ().getVariables
		 * (GlobalGeneratedIdentifiers.EVariableType.SIZE_PARAMETER)) {
		 * VariableDeclarator decl = new VariableDeclarator (new NameID
		 * (varSize.getName ())); decl.setInitializer (new ValueInitializer
		 * (getCommandLineParameterForVariable (varSize))); sl.addStatement (new
		 * DeclarationStatement (new VariableDeclaration
		 * (Globals.SPECIFIER_SIZE, decl))); }
		 */
		...
		for (Variable var : m_data
			...
			decl.setInitializer(new ValueInitializer(new FunctionCall(
					new NameID("atoi"), CodeGeneratorUtil
							.expressions(new ArrayAccess(new NameID("argv"),
									new IntegerLiteral(
											m_mapCommandLineParamIndices
													.get(var)))))));
			sl.addStatement(new DeclarationStatement(new VariableDeclaration(
					Globals.SPECIFIER_SIZE, decl)));
		}
...
/** 
 * Gernerates the allocation code in the driver.c file for all the grid, inclusive the reference grids if needed. 
 */
	@Override
	public StatementList allocateGrids() {
		...
		StatementList sl = new StatementList(new ArrayList<Statement>());
		// Orginal :for (GlobalGeneratedIdentifiers.Variable varGrid :
		// m_data.getData ().getGlobalGeneratedIdentifiers ().getInputGrids ())
		for (GlobalGeneratedIdentifiers.Variable varGrid : m_data
				.getData()
				.getGlobalGeneratedIdentifiers()
				.getVariables(
						EVariableType.INPUT_GRID.mask()
			...
			if (m_data.getArchitectureDescription().useSIMD()
				...
				int nSIMDVectorLength = m_data.getArchitectureDescription()
						.getSIMDVectorLength(specType);

				int nAlignRestrictForType = m_data.getArchitectureDescription()
						.getAlignmentRestriction(specType);
				nAlignRestrict = nAlignRestrict == 1 ? nAlignRestrictForType
						: MathUtil
								.getLCM(nAlignRestrict, nAlignRestrictForType);

				if (nSIMDVectorLength > 1 && nAlignRestrict > 1) {
					Expression exprUnitStrideSize = varGrid.getBoxSize()
							.getCoord(0);

					// create the abort statement (print error message and
					// return)
							...
							.addStatement(new ExpressionStatement(
									new FunctionCall(
											...
											CodeGeneratorUtil.expressions(
													new StringLiteral(
															StringUtil
																	.concat("Non-native, aligned SIMD type mode requires that ",
																			exprUnitStrideSize
																					.toString(),
																			" is divisible by ",
																			nSIMDVectorLength,
					...
					cmpstmtAbort.addStatement(new ReturnStatement(
							new IntegerLiteral(-1)));

					sl.addStatement(new IfStatement(new BinaryExpression(
							new BinaryExpression(exprUnitStrideSize.clone(),
									BinaryOperator.MODULUS, new IntegerLiteral(
											nSIMDVectorLength)),
							BinaryOperator.COMPARE_NE, Globals.ZERO.clone()),
							cmpstmtAbort));
				}
			}

			// get the size of the grid to alloc; extend it if there are
			// alignment restrictions
			Expression exprSize = varGrid.getSize();
			if (nAlignRestrict > 1)
				exprSize = new BinaryExpression(exprSize, BinaryOperator.ADD,
						new IntegerLiteral(nAlignRestrict - 1));

			// add the malloc statement
			Identifier idGrid = m_mapVariableIdentifiers.get(varGrid).clone();

			if (varGrid.getType() == EVariableType.OUTPUT_GRID) {
				sl.addStatement(new ExpressionStatement(
						new AssignmentExpression(
								idGrid,
								AssignmentOperator.NORMAL,
								new Typecast(
								// varGrid.getSpecifiers (),
										ASTUtil.dereference(varGrid
												.getSpecifiers()),
										new FunctionCall(new NameID("malloc"),
												CodeGeneratorUtil
														.expressions(exprSize))))));
			} else {
				sl.addStatement(new ExpressionStatement(
						new AssignmentExpression(idGrid,
								AssignmentOperator.NORMAL, new Typecast(varGrid
										.getSpecifiers(), new FunctionCall(
										new NameID("malloc"), CodeGeneratorUtil
												.expressions(exprSize))))));
			}

			// create the malloc statement for the reference grid (if validation
							...
							new AssignmentExpression(
									new NameID(
											StringUtil.concat(
													...
													ValidationCodeGenerator.SUFFIX_REFERENCE)),
									AssignmentOperator.NORMAL,
									new Typecast(ASTUtil.dereference(varGrid
											.getSpecifiers()),
	...
	public StatementList initializeGrids() {
		...
		NameID nidInitialize = m_data.getData().getGlobalGeneratedIdentifiers()
		...
		List<Variable> listArgVars = m_data
				.getData()
				.getGlobalGeneratedIdentifiers()
				.getVariables(
						~EVariableType.OUTPUT_GRID.mask()
								& ~EVariableType.INTERNAL_AUTOTUNE_PARAMETER
										.mask());

		sl.addStatement(new ExpressionStatement(new FunctionCall(nidInitialize
				.clone(), getFunctionArguments(listArgVars, sl, true))));

		// initialize the reference grids
		if (m_data.getOptions().getCreateValidationCode()) {
			Expression exprInit = new FunctionCall(nidInitialize.clone(),
					getFunctionArguments(listArgVars, sl, false));
			exprInit = (Expression) ASTUtil.addSuffixToIdentifiers(exprInit,
					ValidationCodeGenerator.SUFFIX_REFERENCE, getGridSet());
	...
	/**
	 * This method generates the initialization code for the grids used for the
	 * computation
	 * 
	 * @return
	 */
	...
	public StatementList computeStencil() {
		...
		// if the code is compatible with Fortran, don't include the output
		// grids
		...
		sl.addStatement(new ExpressionStatement(new FunctionCall(m_data
				.getData().getGlobalGeneratedIdentifiers()
				.getStencilFunctionName().clone(), listArgs)));
	...
	public StatementList validateComputation() {
		...
		for (Variable v : m_data.getData().getGlobalGeneratedIdentifiers()
				.getVariables())
			if (v.getType() == EVariableType.KERNEL_PARAMETER)
				mapVariables.put(v.getName(), getParameterSampleValue(v));

		for (Statement stmt : sl) {
			for (DepthFirstIterator it = new DepthFirstIterator(stmt); it
					.hasNext();) {
				Object o = it.next();
				if (o instanceof IDExpression) {
					Expression exprValue = mapVariables.get(((IDExpression) o)
							.getName());
					if (exprValue != null)
	...
	/**
	 * Returns a global alignment restriction that is valid for all types of the
	 * used variables in <code>listVariables</code>.
	 * 
	 * @param listVariables
	 *            The list of variables for which to determine the common
	 *            alignment restriction
	 * @return The alignment restriction common to the types of the variables in
	 *         <code>listVariables</code>
	 */
	private int getGlobalAlignmentRestriction(List<Variable> listVariables) {
		...
		for (Specifier s : setTypes) {
			int nAlignRestrForType = m_data.getArchitectureDescription()
					.getAlignmentRestriction(s);
			nAlignRestrict = nAlignRestrict == 1 ? nAlignRestrForType
					: MathUtil.getLCM(nAlignRestrict, nAlignRestrForType);
		}

		return nAlignRestrict;
	}

	private Expression getArgumentExpressionFromVariable(Variable v,
			StatementList slAuxStatements, int nAlignRestrict,
			boolean bForceAlign) {
		...
		Expression exprArg = getExpressionForVariable(v,
				EOutputGridType.OUTPUTGRID_POINTER);
		if (v.getType() == EVariableType.INPUT_GRID && nAlignRestrict > 1) {
			if (bForceAlign) {
				// force the array to be aligned at nAlignRestrict boundaries

				if (bIsPowerOfTwo) {
					// (type*) (((uint_ptr) <ptr> + (align_restrict - 1)) &
					// ~(align_restrict - 1))
					return new Typecast(
							v.getSpecifiers(),
					...
					return new Typecast(v.getSpecifiers(),
							new BinaryExpression(ExpressionUtil.ceil(
									new Typecast(CodeGeneratorUtil
											.specifiers(new UserSpecifier(
													new NameID("uintptr_t"))),
											exprArg.clone()),
									new IntegerLiteral(nAlignRestrict), false),
									BinaryOperator.MULTIPLY,
			...
			if (bIsFortranCompatible) {
				if (!(exprArg instanceof IDExpression)) {
					VariableDeclarator decl = new VariableDeclarator(
							new NameID(StringUtil.concat("__tmparg_",
									m_nTmpArgCount++)));
					decl.setInitializer(new ValueInitializer(exprArg.clone()));
					slAuxStatements.addStatement(new DeclarationStatement(
							new VariableDeclaration(v.getSpecifiers(), decl)));
	...
	/**
	 * Creates a list of arguments to the <code>initialize</code> and the
	 * <code>kernel</code> functions. Takes care of alignment restrictions.
	 * 
	 * @return List of function arguments to <code>initialize</code> and
	 *         <code>kernel</code>
	 */
	private List<Expression> getFunctionArguments(List<Variable> listVariables,
		...
		// build the list of arguments; adjust the pointers so that the
		// alignment restrictions are satisfied
		List<Expression> listArgs = new ArrayList<>();
		for (Variable v : listVariables)
			listArgs.add(getArgumentExpressionFromVariable(v, sl,
					nAlignRestrict, bForceAlign));

		return listArgs;
	}

	private Set<IDExpression> getGridSet() {
		List<Variable> listVars = m_data
				.getData()
				.getGlobalGeneratedIdentifiers()
				.getVariables(
						GlobalGeneratedIdentifiers.EVariableType.INPUT_GRID
								.mask()
								| GlobalGeneratedIdentifiers.EVariableType.OUTPUT_GRID
										.mask());
	...
	public StatementList deallocateGrids() {
		...
		for (Variable v : m_data.getData().getGlobalGeneratedIdentifiers()
			...
			if (m_data.getOptions().getCreateValidationCode()) {
				...
				exprVarRef = (Expression) ASTUtil.addSuffixToIdentifiers(
						exprVarRef, ValidationCodeGenerator.SUFFIX_REFERENCE,
						setGrids);
				sl.addStatement(new ExpressionStatement(new FunctionCall(
						new NameID("free"), CodeGeneratorUtil
								.expressions(exprVarRef))));
			}
		}
	...
	public StatementList writeGrids(String strFilenameFormat, String strType) {
		...
		byte nDimensionality = m_data.getStencilCalculation()
				.getDimensionality();
		...
		boolean bIsFortran = m_kernelSourceFile.getCompatibility() == CodeGenerationOptions.ECompatibility.FORTRAN;
		int nMask = bIsFortran ? ~EVariableType.OUTPUT_GRID.mask() : ~0;
		int nAlignRestrict = getGlobalAlignmentRestriction(m_data
				.getData()
				.getGlobalGeneratedIdentifiers()
				.getVariables(
						~EVariableType.INTERNAL_NONKERNEL_AUTOTUNE_PARAMETER
		...
		for (GlobalGeneratedIdentifiers.Variable varGrid : m_data.getData()
				.getGlobalGeneratedIdentifiers().getVariables(type)) {
			Specifier specType = varGrid.getSpecifiers().get(0);
			NameID nidWriteFnx = new NameID(
					specType.equals(Specifier.FLOAT) ? "write_data_f"
							: "write_data_d");

			String strFilename = strFilenameFormat.replace("%s",
					varGrid.getName());
			...
			listArgs.add(bIsInput ? getArgumentExpressionFromVariable(varGrid,
					sl, nAlignRestrict, true) : new NameID(varGrid.getName()));
	...
	public Expression getFlopsPerStencil() {
		return new IntegerLiteral(m_data.getStencilCalculation()
				.getStencilBundle().getFlopsCount());
	}
	...
	public Expression getGridPointsCount() {
		...
		rgExprPointsCount[0] = m_data.getStencilCalculation()
				.getMaxIterations().clone();
		for (int i = 0; i < nDim; i++) {
			rgExprPointsCount[i + 1] = ExpressionUtil
					.increment(new BinaryExpression(m_data
							.getStencilCalculation().getDomainSize().getMax()
							.getCoord(i).clone(), BinaryOperator.SUBTRACT,
							m_data.getStencilCalculation().getDomainSize()
									.getMin().getCoord(i).clone()));
		}
		return ExpressionUtil.product(rgExprPointsCount);
	}

	/**
	 * Returns the number of bytes in the memory objects corresponding to the
	 * stencil nodes <code>nodes</code>.
	 * 
	 * @param nodes
	 *            The nodes for which to look for the memory objects and in
	 *            which to count bytes
	 * @return The number of bytes in the memory objects corresponding to the
	 *         stencil nodes in <code>nodes</code>
	 */
	private Expression getBytes(Iterable<StencilNode> nodes) {
		Expression exprTotalBytes = null;

		SubdomainIdentifier sdidRoot = m_data.getCodeGenerators()
				.getStrategyAnalyzer().getRootSubdomain();
		Map<MemoryObject, StencilNode> mapMemoryObjects = new HashMap<>();

		for (StencilNode node : nodes)
			mapMemoryObjects.put(m_data.getData().getMemoryObjectManager()
					.getMemoryObject(sdidRoot, node, true), node);

		for (MemoryObject mo : mapMemoryObjects.keySet()) {
			Expression exprMemoryObjectBytes = new BinaryExpression(mo
					.getSize().getVolume(), BinaryOperator.MULTIPLY,
					new SizeofExpression(
							CodeGeneratorUtil.specifiers(mapMemoryObjects.get(
									mo).getSpecifier())));

			if (exprTotalBytes == null)
				exprTotalBytes = exprMemoryObjectBytes;
			else
				exprTotalBytes = new BinaryExpression(exprTotalBytes.clone(),
						BinaryOperator.ADD, exprMemoryObjectBytes);
		}
	...
	public Expression getBytesTransferred() {
		Expression exprReadBytes = getBytes(m_data.getStencilCalculation()
				.getStencilBundle().getFusedStencil()/*
													 * , new IntegerLiteral (0)
													 */); // !!! XXX !!!
															// timeblocking!!???
		Expression exprWriteBytes = getBytes(m_data.getStencilCalculation()
				.getStencilBundle().getFusedStencil().getOutputNodes()/*
																	 * , //new
																	 * IntegerLiteral
																	 * (-1)); //
																	 * without
		...
		Expression exprBytes = exprWriteBytes == null ? exprReadBytes
				: new BinaryExpression(exprReadBytes, BinaryOperator.ADD,
						exprWriteBytes);
		return new BinaryExpression(m_data.getStencilCalculation()
				.getMaxIterations().clone(), BinaryOperator.MULTIPLY, exprBytes);
	}

	@Override
	public Expression getDoValidation() {
		return (m_data.getOptions().getCreateValidationCode() ? Globals.ONE
				: Globals.ZERO).clone();
	}
	...
	public Expression getValidates() {
		...
		Expression exprHasValidationErrors = m_cgValidate
				.getHasValidationErrors();
		if (exprHasValidationErrors == null)
			return Globals.ONE.clone();

		return new UnaryExpression(UnaryOperator.LOGICAL_NEGATION,
				exprHasValidationErrors.clone());
	}
	...
	public String getExeParams() {
		List<Variable> listVariables = m_data
				.getData()
				.getGlobalGeneratedIdentifiers()
				.getVariables(
						GlobalGeneratedIdentifiers.EVariableType.SIZE_PARAMETER
		...
		for (Variable var : listVariables) {
			if (var.getType().equals(
				...
				IAutotunerParam param = m_data.getStrategy()
						.getAutotuneSpecification(var.getName());
				if (param == null)
					param = var.getAutotuneParam();

				if (param != null) {
					// append the auto-tune specification
					// (any size parameters replaced by references to the
					// corresponding command line argument)
					sb.append('@');
					sb.append(m_data.getData().getGlobalGeneratedIdentifiers()
							.getDefinedVariableName(var));
					sb.append('=');
					sb.append(param.replaceIdentifiers(mapArgName2ArgIdx)
							.replaceAll("\\(", "\\\\\\(")
							.replaceAll("\\)", "\\\\\\)"));
				} else {
					// no auto-tune specification found; just add the variable
					// to the command line
					sb.append("$(");
					sb.append(var.getName());
					sb.append(')');
