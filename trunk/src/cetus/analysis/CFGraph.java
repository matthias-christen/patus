...
public class CFGraph extends DFAGraph
{
  ...
  public void normalize()
  {
    ...
    do
    {
      ...
      {
        List<SymbolTable> symbol_exit = temp.getLast().getData("symbol-exit");

        if ( symbol_exit != null )
          for (Object st : (List)data)
            symbol_exit.add((SymbolTable)st);

        else
        {
          symbol_exit = new ArrayList<SymbolTable>();
          for (Object st : (List)data)
            symbol_exit.add((SymbolTable)st);
  ...
  protected DFANode buildGoto(GotoStatement stmt)
  {
    ...
    if (goto_link.get(label) == null)
      goto_link.put(label, new LinkedList<DFANode>());
  ...
  protected DFAGraph buildSwitch(SwitchStatement stmt)
  {
    ...
    // Delay links.
    break_link.push(new ArrayList<DFANode>());
    switch_link.push(new ArrayList<DFANode>());
    ...
    for ( DFANode from : break_link.pop() )
      ret.addEdge(from, exit);
    for ( DFANode to : switch_link.pop() )
      ret.addEdge(entry, to);
...
*/
  ...
  protected void expandAssignment
  ...
  protected void expandConditional
  (CompoundStatement stmts, ConditionalExpression e)
  {
    ...
    Identifier id = SymbolTools.getTemp(stmts, spec, null);

    // Build an if statement.
    AssignmentOperator op = AssignmentOperator.NORMAL;
    Expression etrue =
        new AssignmentExpression(id, op, ce.getTrueExpression().clone());
    Expression efalse =
        new AssignmentExpression((Expression)id.clone(), op,
        ce.getFalseExpression().clone());
    Statement ifstmt = new IfStatement(ce.getCondition().clone(),
  ...
  protected void expandUnary(CompoundStatement stmts, UnaryExpression e)
  {
    ...
    if ( op==UnaryOperator.POST_DECREMENT || op==UnaryOperator.PRE_DECREMENT )
      rhs = new BinaryExpression(var.clone(),
  ...
  protected static void removeRedundancy(CompoundStatement stmts)
  {
    ...
    for ( int i=stmt_list.size()-1; i>=0; --i )
    {
      ...
      else if ( e instanceof AssignmentExpression )
      {
        ...
        if ( to instanceof Identifier )
        {
          ...
          if ( SymbolTools.getSymbols(stmts).contains(var) && !lives.contains(var) )
          {
            stmts.removeChild(stmt);
            kills.add(var);
            continue;
      ...
      for ( Symbol var : SymbolTools.getSymbols(stmts) )
    ...
    Iterator<Traversable> iter = stmts.getChildren().iterator();
    while ( iter.hasNext() )
    {
      Traversable o = iter.next();
      for ( Symbol kill : kills )
        if ( IRTools.containsSymbol(o, kill) )
        {
          iter.remove();
          break;
        }
