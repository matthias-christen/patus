 ...
 * @see Domain
 */
public class PointsToDomain 
	extends LinkedHashMap<Symbol,HashSet<PointsToRel>> 
	implements Domain
{
  ...
  @Override
	...
	public Domain union(Domain other)
	{
		...
		if ( other instanceof PointsToDomain ) {
			if ( ((PointsToDomain)other).isEmpty() )
				return ret;
			if (this.isEmpty())
				return ((PointsToDomain)other).clone();

			for (Symbol s : ((PointsToDomain)other).keySet()) {
				if ( this.containsKey(s) )
				{
					HashSet<PointsToRel> p1_set = this.get(s);
					HashSet<PointsToRel> p2_set = ((PointsToDomain)other).get(s);
					HashSet<PointsToRel> merged_set =
						new LinkedHashSet<PointsToRel>();
					merged_set.addAll(this.get(s));
					merged_set.addAll(((PointsToDomain)other).get(s));
				...
				{
					PointsToDomain temp = ((PointsToDomain)other).clone();
	 ...
	 * @return merged domain
	 */
	public static Domain merge(Domain s1, 
			Domain s2)
	{
		Domain merged = null;
		if (s1 instanceof PointsToDomain &&
				s2 instanceof PointsToDomain)
		{
			...
			for (Symbol s : union.keySet())
			{
				...
				HashSet<PointsToRel> definite_rels = 
					definite_set.get(s);
		...
		else if (s1 instanceof Universe ||
				s2 instanceof Universe)
			merged = Universe.getUniverse();
		else if (s1 instanceof NullDomain &&
				s2 instanceof PointsToDomain)
			merged = ((PointsToDomain)s2).clone();
		else if (s1 instanceof PointsToDomain &&
				s2 instanceof NullDomain)
	...
	public Domain intersect(Domain other)
	{
		...
		if (other instanceof PointsToDomain)
		{
			...
			Set<Symbol> s1_set = ((PointsToDomain)other).keySet();
			for (Symbol s1 : s1_set)
			{
				HashSet<PointsToRel> rel1_set = ((PointsToDomain)other).get(s1);
					...
					for (PointsToRel p1_rel : rel1_set)
					{
						for (PointsToRel p2_rel : rel2_set)
						{
							if (p1_rel.equals(p2_rel)) {
								...
								((PointsToDomain)ret).addRel(intersect_rel);
							}
						}
					}
				}
	...
	public Domain diffStrong(Domain other)
	{
		...
		if ( other instanceof PointsToDomain )
		{
			ret = this.clone();
			Set<Symbol> keys = 
				((PointsToDomain)other).keySet();
			for (Symbol s : keys)
			{
				HashSet<PointsToRel> other_p_rel = ((PointsToDomain)other).get(s);
				...
				if (this_p_rel != null)
				{
					for (PointsToRel p1_rel : other_p_rel)
					{
						for (PointsToRel p2_rel : this_p_rel)
						{
							if (p1_rel.equals(p2_rel))
							{
								HashSet<PointsToRel> ret_p_rel = 
									((PointsToDomain)ret).get(s);
	...
	{
		Domain kill_set = new PointsToDomain();
		for (Symbol s : this.keySet())
		{
			if (exps.contains(s))
				((PointsToDomain)kill_set).put(s,
						this.get(s));
		}
		return kill_set;
	}
    ...
    StringBuilder str = new StringBuilder(16);
		str.append("(");
		str.append(rel.getPointerSymbol().getSymbolName());
		str.append(",");
		str.append(rel.getPointedToSymbol().getSymbolName());
		if ( rel.isDefinite() )
			str.append(",D)");
		else
			str.append(",P)");
