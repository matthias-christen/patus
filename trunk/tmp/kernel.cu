 ...
 * 
 * This code was generated by Patus on 2011/08/08 17:56:18
 */
...
__device__ void laplacian___unroll_p3_20201(float *  *  u_0_1_out, float *  u_0_0, float *  u_0_1, float alpha, float beta, int x_max, int y_max, int z_max, int cb_x, int cb_y, int cb_z, int chunk)
{
	...
	*/
	...
	Implementation
	*/
	start0=((blockDim.x*blockIdx.x)+threadIdx.x);
	end0=(((int)(((((int)(((x_max+cb_x)-1)/cb_x))+chunk)-1)/chunk))-1);
	numthds0=(blockDim.x*gridDim.x);
	start1=((blockDim.y*blockIdx.y)+threadIdx.y);
	end1=(((int)(((int)(((y_max+cb_y)-1)/cb_y))/1))-1);
	numthds1=(blockDim.y*gridDim.y);
	start2=((blockDim.z*0)+threadIdx.z);
	end2=(((int)(((z_max+cb_z)-1)/cb_z))-1);
	numthds2=(blockDim.z*gridDim.z);
	/*
	for v2_idx = start2..end2 by numthds2 parallel 1 <level 1> schedule 1 { ... }
	*/
	for (v2_idx=start2; v2_idx<=end2; v2_idx+=numthds2)
	{
		v2_idx_z=v2_idx;
		v2_idx_z=((v2_idx_z*cb_z)+1);
		v2_idx_z_max=min((v2_idx_z+cb_z), (z_max+1));
		/*
		for v2_idx = start1..end1 by numthds1 parallel 1 <level 1> schedule 1 { ... }
		*/
		for (v2_idx=start1; v2_idx<=end1; v2_idx+=numthds1)
		{
			v2_idx_y=v2_idx;
