 ...
 * 
 * This code was generated by Patus on 2011/08/04 15:15:43
 */
...
__device__ void laplacian___unroll_p3_20201(float *  *  u_0_1_out, float *  u_0_0, float *  u_0_1, float alpha, float beta, int x_max, int y_max, int z_max, int cb_x, int cb_y, int cb_z, int chunk)
{
	...
	Initializations
	*/
	stride_1=((int)(((blockDim.y+y_max)-1)/blockDim.y));
	tmp=blockIdx.y;
	idx_1_2=(tmp/stride_1);
	tmp-=(idx_1_2*stride_1);
	idx_1_1=tmp;
	v2_idx_x_max=(v2_idx_x+(cb_x*chunk));
	...
	*/
	/*
	for v2_idx_x = (1+(((blockDim.x*blockIdx.x)+threadIdx.x)*(cb_x*chunk)))..((1+(((blockDim.x*blockIdx.x)+threadIdx.x)*(cb_x*chunk)))+(chunk-1)) by 1 parallel 1 <level 1> schedule  { ... }
	*/
	for (v2_idx_x=(1+(((blockDim.x*blockIdx.x)+threadIdx.x)*(cb_x*chunk))); v2_idx_x<=((1+(((blockDim.x*blockIdx.x)+threadIdx.x)*(cb_x*chunk)))+(chunk-1)); v2_idx_x+=1)
	{
...
__global__ void initialize_laplacian(float *  u_0_0, float *  u_0_1, float alpha, float beta, int x_max, int y_max, int z_max, int cb_x, int cb_y, int cb_z, int chunk)
{
	...
	*/
	...
	v2_idx_z=(1+(((blockDim.z*idx_1_2)+threadIdx.z)*cb_z));
	v2_idx_z_max=(v2_idx_z+cb_z);
	v2_idx_x_max=(v2_idx_x+(cb_x*(chunk*chunk)));
	/*
	Implementation
	*/
	/*
	for v2_idx_x = (((cb_x*(chunk*chunk))*(threadIdx.x+(blockDim.x*blockIdx.x)))+1)..((((cb_x*(chunk*chunk))*(threadIdx.x+(blockDim.x*blockIdx.x)))+1)+(chunk-1)) by 1 parallel 1 <level 1> schedule  { ... }
	*/
	for (v2_idx_x=(((cb_x*(chunk*chunk))*(threadIdx.x+(blockDim.x*blockIdx.x)))+1); v2_idx_x<=((((cb_x*(chunk*chunk))*(threadIdx.x+(blockDim.x*blockIdx.x)))+1)+(chunk-1)); v2_idx_x+=1)
	{
		if ((((v2_idx_x<=x_max)&&(v2_idx_y<=y_max))&&(v2_idx_z<=z_max)))
		{
			/* Index bounds calculations for iterators in v2[t=t][0] */
