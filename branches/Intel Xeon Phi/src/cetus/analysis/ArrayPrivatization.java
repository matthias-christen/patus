...
public class ArrayPrivatization extends AnalysisPass
{
	...
	// Pass name
	private static final String tag = "[Array Privatization]";
	...
	public void analyzeProcedure(Procedure proc)
	{
		...
		PrintTools.printlnStatus(tag+" Procedure \""+proc.getName()+"\" ...", 1);
			...
			for ( Symbol var : keys )
			{
				...
				for ( Object aliased : alias_set )
				{
					...
					else if ( var != aliased )
					{
						...
						PrintTools.printlnStatus(tag+"  Removing aliased variable ("+
							var+","+aliased+")", 1);
	...
	private void analyzeLoop(Loop loop)
	{
		...
		current_loop = loop;
		loop_variants = DataFlowTools.getDefSymbol((Traversable)current_loop);
		pri_map.put(loop, new LinkedHashSet<Symbol>());

		PrintTools.printlnStatus("\n"+tag+" Loop: "+LoopTools.toControlString(loop), 2);
	...
	private CFGraph buildLoopGraph()
	{
		...
		PrintTools.printlnStatus(ret.toDot("range,ir,tag", 2), 5);
	...
	private void addUseFromDeclaration()
	{
		...
		RangeDomain rd = range_map.get((Statement)current_loop);
		if ( rd == null )
			rd = new RangeDomain();
		else
			rd = rd.clone();
		DepthFirstIterator iter =
			new DepthFirstIterator(current_loop.getBody());
		iter.pruneOn(Loop.class);
			...
			if ( o instanceof Initializer )
				for ( Object child : ((Initializer)o).getChildren() )
					if ( child instanceof Traversable )
						for ( Expression use : DataFlowTools.getUseSet((Traversable)child) )
	 ...
	 *          = T   otherwise 
	 */
	private Map<Symbol, Expression>
			joinInductionMap(Map<Symbol, Expression> m1, Map<Symbol, Expression> m2)
	{
	...
	private void meetInductionMap(DFANode node)
	{
		...
		while ( iter.hasNext() )
		{
			...
			if ( var != null )
			{
				if ( IRTools.containsClass((Traversable)o, ConditionalExpression.class) )
	...
	private Set<Symbol> getMayUseFromFC(Traversable tr, Set<Symbol> vars)
	{
		...
		while ( iter.hasNext() )
		{
			...
			if ( o instanceof FunctionCall )
			{
				...
				{
					if ( IRTools.containsSymbol((Traversable)o, var) )
						ret.add(var);
				}
			}
		}
	...
	private void collectPrivateSet(CFGraph g)
	{
		...
		while ( iter.hasNext() )
		{
			...
			if ( o instanceof Loop )
			{
				Set<Symbol> inner_pri = new LinkedHashSet<Symbol>(pri_map.get((Loop)o));
				inner_pri.removeAll(use_set);
				//inner_pri.removeAll(def_set); TODO: check why this was necessary.
		...
		Set local_set =
			SymbolTools.getVariableSymbols((SymbolTable)current_loop.getBody());
		...
		// Variables that may be defined in inner loops need to be tested if there
		// is any read access to these variables. If there is no use point for
		// those variables, they are still considered to be private.
		...
		PrintTools.printlnStatus(tag+" DEF = "+def_map.get(current_loop), 2);
		PrintTools.printlnStatus(tag+" USE = "+use_map.get(current_loop), 2);
		PrintTools.printlnStatus(tag+" PRI = "+pri_map.get(current_loop), 2);
	}
	...
	{
		DepthFirstIterator iter = new DepthFirstIterator((Traversable)current_loop);
		...
		if ( fc != null )
		{
			...
			for ( Symbol var : keys )
			{
				if ( SymbolTools.isGlobal(var, (Traversable)current_loop) )
					vars.remove(var);
			}
		}
	}
	...
	(DFANode node, Set<Symbol> gen, Set<Symbol> kill, Set<Symbol> mask_set)
	{
	...
	private void aggregateDef()
	{
		...
		RangeDomain entry_range = range_map.get((Statement)current_loop);
		if ( entry_range == null )
		...
		if ( current_loop instanceof ForLoop )
		{
			...
			init_set.addAll(DataFlowTools.getDefSymbol(for_loop.getInitialStatement()));
		}
		else if ( current_loop instanceof WhileLoop )
		{
		...
		PrintTools.printlnStatus(tag+" DEFS = "+defs+" under "+exit_range, 2);
	}
	...
	private void aggregateUse()
	{
		...
		PrintTools.printlnStatus(tag+" USES = "+my_use+" under "+exit_range, 2);
	}
	...
	private Section.MAP getSectionMap(Expression e, boolean def)
	{
		...
		else
		{
			...
			// e.g.) *a = 0;
			// TODO: compare AccessSymbol and NewAccessSymbol.
			if ( var != null )
	...
	private void getRangeDomain(CFGraph g)
	{
		...
		for ( Integer order : work_list.keySet() )
		{
			...
			{
				RangeDomain range = range_map.get((Statement)current_loop);
