...
public class StandardLibrary
{
  ...
  */
  public static boolean isSideEffectFreeExceptIO(FunctionCall fcall)
  {
    ...
    Set<Property> properties = std.catalog.get(fcall.getName().toString());
    return (
      properties.contains(Property.SIDE_EFFECT_STDIO) ||
  ...
  /**
  * Adds each entry to the repository. The assigned properties are based on the
  * description of the standard library functions that should have a function
  * declarator (456 entries), following C99 (ISO/IEC 9899:1999).
  */
  ...
  private static int getNumMutableArguments(ProcedureDeclarator pdecl)
  {
    ...
    for ( Declaration decl : pdecl.getParameters() )
    {
      ...
      if ( SymbolTools.isPointer(param) && !types.contains(Specifier.CONST) )
        ret++;
    }
    return ret;
  }
  ...
  private static boolean containsStreamArguments(ProcedureDeclarator pdecl)
  {
    ...
    for ( Declaration decl : pdecl.getParameters() )
    {
      ...
      for ( Object o : param.getTypeSpecifiers() )
        if ( o.toString().equals("FILE") || o.toString().equals("__FILE"))
          return true;
    }
    return false;
  }
