...
public class CetusAnnotationParser
{
	...
	private static boolean check_prev_token(String istr)
	{
		if ( start_of_token() )
			return false;
		return ( token_array[token_index-1].compareTo(istr) == 0 ) ? true : false;
	} 
	...
	{
		return (token_index == 0) ? true : false;
	}
	...
	{
		PrintTools.println("CetusAnnotationParser is parsing [parallel] clause", 2);
		...
		while (end_of_token() == false) 
		{
			...
			default : CetusAnnotationParserError("NoSuchParallelConstruct : " + clause);
			}
		}
	}
	...
	{
		PrintTools.println("CetusAnnotationParser is parsing [private] clause", 2);
		match("(");
		Set<String> set = new HashSet<String>();
		parse_commaSeparatedList(set);
		match(")");
	...
	{
		PrintTools.println("CetusAnnotationParser is parsing [firstprivate] clause", 2);
		match("(");
		Set<String> set = new HashSet<String>();
		parse_commaSeparatedList(set);
		match(")");
	...
	{
		PrintTools.println("CetusAnnotationParser is parsing [lastprivate] clause", 2);
		match("(");
		Set<String> set = new HashSet<String>();
		parse_commaSeparatedList(set);
		match(")");
	...
	{
		PrintTools.println("CetusAnnotationParser is parsing [reduction] clause", 2);
		...
		if ( match(":")==false ) 
		{
			CetusAnnotationParserError("colon expected before a list of reduction variables");
		}
	...
	private static void addToMap(String key, String new_str)
	{
		if (cetus_map.keySet().contains(key))
			Tools.exit("[Warning] Cetus Annotation Parser detected duplicate pragma: " + key);
		else
