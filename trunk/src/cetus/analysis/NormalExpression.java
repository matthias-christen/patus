...
public class NormalExpression implements Comparable,Comparator<NormalExpression>
{
  ...
  public static Expression negate(Expression e)
  {
    Expression e1 = new UnaryExpression(
			UnaryOperator.LOGICAL_NEGATION, e.clone());
...
//------------------------------------------------------------------------------
	...
	public static void runtest(Program prog)
	{
		...
		while ( iter.hasNext() )
		{
			...
			else if ( t instanceof AssignmentExpression )
			{
				...
				PrintTools.printlnStatus("ID = "+id_list+", EXPR = "+e, 0);
				for ( Identifier id : id_list )
					PrintTools.printlnStatus("  COEF["+id+"] = "+getCoefficient(e, id), 0);
				PrintTools.printlnStatus("  CONST   = "+getConstantCoefficient(e, id_list), 0);
				PrintTools.printlnStatus("  COEF    = "+getCoefficient(e, id_list), 0);
				PrintTools.printlnStatus("  VARS    = "+getVariableList(e), 0);
				PrintTools.printlnStatus("  AFFINE  = "+isAffine(e, id_list), 0);
				PrintTools.printStatus("  POLY["+id_list.getFirst()+"] = ", 0);
  ...
  private NormalExpression mod()
  {
    ...
    if (num2 != null)
    {
        ...
        // cancel out constants if possible
    	if (ne1.op == OP.ADD)
    	{
    		NormalExpression ne1_new = new NormalExpression (OP.ADD);
    		for (Object o1 : ne1.list)
    		{
    			Expression expr1 = ((NormalExpression) o1).getExpression ();
    			if (expr1 instanceof IntegerLiteral)
    				ne1_new.add (new NormalExpression (new IntegerLiteral (((IntegerLiteral) expr1).getValue () % num2)));
    			else
    				ne1_new.add ((NormalExpression) o1);
    		}
    		
    		ne1 = ne1_new.simplify ("SIMP");
    		ret.list.set (0, ne1);
    	}
    	
        // replace modulo by 2^n by (& (2^n-1))
    	boolean bIsPowerOf2 = false;
    	...
    	if (bIsPowerOf2)
    		return new NormalExpression (new BinaryExpression (ne1.getExpression (), BinaryOperator.BITWISE_AND, new IntegerLiteral (num2 - 1)));
    }
    //<-- MCH end
